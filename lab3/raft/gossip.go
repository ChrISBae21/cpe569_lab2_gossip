...
func AfterYSeconds(node *Node) {
	// Send heartbeat table to random neighbor
	neighborId := rand.Intn(nodeCount)
	for neighborId == node.id {
		neighborId = rand.Intn(nodeCount)
	}

	neighbor := node.neighbors[neighborId]

	select {
	case neighbor.gossip_info.channel <- copyTable(node.gossip_info.hbTable):
		// sent successfully
	default:
		// buffer full
	}
...
func MergeTable(node *Node, receivedTable HeartbeatTable) {
	...
	for id, received_hb := range receivedTable {
		stored_hb, exists := node.gossip_info.hbTable[id]
		if exists {
			...
			if received_hb.counter > stored_hb.counter {
				...
				node.gossip_info.hbTable[id] = received_hb
			}
		} else {
...
func PrintTable(node *Node) {
	...
	fmt.Println("  ---|---------|---------------|-------")
	for id, entry := range node.gossip_info.hbTable {
